...
__global__ void gpu_grayscale(int width, int height, float *image, float *image_out)
{
    ...
    {
        int offset_out = index_y * width + index_x; // 1 color per pixel
        int offset     = offset_out * 3;            // 3 colors per pixel
        
        // Convert to grayscale following the "luminance" model
        image_out[offset_out] = image[offset]     * 0.0722f + // B
                                image[offset + 1] * 0.7152f + // G
                                image[offset + 2] * 0.2126f;  // R
    }
}
...
{
    __shared__ float block[BLOCK_SIZE_SH * BLOCK_SIZE_SH];
    ...
    if (index_x < width && index_y < height)
    {
        int offset_b = threadIdx.y * BLOCK_SIZE_SH + threadIdx.x;
        int offset_t = index_y * width + index_x;
        
        block[offset_b] = image[offset_t];
        
        __syncthreads();
        
        if (threadIdx.x == (BLOCK_SIZE - 1) && index_x < (width - 2))
        {
            block[offset_b + 1] = image[offset_t + 1];
            block[offset_b + 2] = image[offset_t + 2];
        }
        
        if (threadIdx.y == (BLOCK_SIZE - 1) && index_y < (height - 2))
        {
            int offset_b_1 = (threadIdx.y + 1) * BLOCK_SIZE_SH + threadIdx.x;
            int offset_b_2 = (threadIdx.y + 2) * BLOCK_SIZE_SH + threadIdx.x;
            int offset_t_1 = (index_y + 1) * width + index_x;
            int offset_t_2 = (index_y + 2) * width + index_x;
            
            block[offset_b_1] = image[offset_t_1];
            block[offset_b_2] = image[offset_t_2];
            
            if (threadIdx.x == (BLOCK_SIZE - 1) && index_x < (width - 2))
            {
                block[offset_b_1 + 1] = image[offset_t_1 + 1];
                block[offset_b_1 + 2] = image[offset_t_1 + 2];
                block[offset_b_2 + 1] = image[offset_t_2 + 1];
                block[offset_b_2 + 2] = image[offset_t_2 + 2];
            }
        }
        ...
        if (index_x < (width - 2) && index_y < (height - 2))
        {
            ...
            image_out[offset] = cpu_applyFilter(&block[offset_b],
...
__global__ void gpu_sobel(int width, int height, float *image, float *image_out)
{
    ...
    if (index_x < width && index_y < height)
    {
        ...
        if (index_x < (width - 2) && index_y < (height - 2))
        {
            ...
            float gx = cpu_applyFilter(&block[offset_b],
                                       BLOCK_SIZE_SH, sobel_x, 3);
            float gy = cpu_applyFilter(&block[offset_b],
